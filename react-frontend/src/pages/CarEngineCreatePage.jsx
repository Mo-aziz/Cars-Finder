...
const CarEngineCreatePage = () => {
  ...
  const fetchData = async () => {
    try {
      const [carsData, enginesData] = await Promise.all([
        carService.getAll(),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await carEngineService.create(formData)
      navigate('/car-engines')
    } catch (err) {
      setError(err.message || 'Failed to create car engine')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="container">
      ...
      <div className="card max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold mb-6">Add Car Engine</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {submitting ? 'Creating...' : 'Create Car Engine'}
            </button>
...
export default CarEngineCreatePage
